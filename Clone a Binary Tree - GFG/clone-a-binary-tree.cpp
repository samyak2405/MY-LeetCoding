...
class Solution{
  public:
    ...
    Node *preorder(Node *tree,unordered_map<Node *,Node *> &mp)
    {
        if(!tree)
            return NULL;
        mp[tree] = new Node(tree->data);
        mp[tree]->left = preorder(tree->left,mp);
        mp[tree]->right = preorder(tree->right,mp);
        return mp[tree];
    }
    
    void randomAssign(Node *root,unordered_map<Node *,Node *> &mp)
    {
        if(!root)
            return;
        mp[root]->random = mp[root->random];
        randomAssign(root->left,mp);
        randomAssign(root->right,mp);
    }
    ...
    Node* cloneTree(Node* tree)
    {
        ...
        Node *root = preorder(tree,mp);
        randomAssign(tree,mp);
        return root;
    }
};
